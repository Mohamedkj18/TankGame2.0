...
Tank::Tank(int x, int y, Direction dir, Game *game, int playerId)
    : GameObject(x, y, dir, game), playerId(playerId);

int Tank::getplayerId() { return playerId; }

int Tank::getTankId() { return id; }

ActionRequest Tank::getLastMove() { return lastMove; }

void Tank::setLastMove(ActionRequest lastMove)
{
    lastMove = lastMove;
}
...
void Tank::fire()
{
    ...
    {
        std::unique_ptr<Shell> shell = std::make_unique<Shell>(x, y, direction, game);
        shell->moveForward();
        game->addShell(shell);
    }
}
