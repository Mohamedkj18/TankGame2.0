...
MyTankAlgorithm::MyTankAlgorithm(int numMovesPerUpdate, int range)
    : moveIndex(0), range(range), maxMovesPerUpdate(numMovesPerUpdate) {}
...
void MyTankAlgorithm::updateBattleInfo(BattleInfo& info) {
    ...
    role = myInfo.getRole();
    threats = myInfo.getThreats();
    nearbyFriendlies = myInfo.getFriendlies();
...
void MyTankAlgorithm::prepareActions() {
    ...
    for (auto& nextPos : bfsPath) {
        if (steps >= maxMovesPerUpdate) break;
